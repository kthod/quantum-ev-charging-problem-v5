...
def floor_to_set(x: float, allowable_set: np.array, eps=0.05):
...
def increment_in_set(x: float, allowable_set: np.array):
...
def project_into_discrete_feasible_pilots(
    rates: np.ndarray, allowable_set: np.array,d :np.array
):
    ...
    for i in range(N):
        for t in range(T):
            if t<d[i]:  
                 new_rates[i, t] = floor_to_set(rates[i, t], allowable_set, eps=0.05)
            else:
                new_rates[i, t] = 0
...
def diff_based_reallocation(
    ...
    for t in range(T):
        ...
        rounded_rates = project_into_discrete_feasible_pilots(rates, allowable_set,d)

        def metric(i):
            return -(init_rates[i] - rounded_rates[i, t])

        sorted_indexes = sorted(range(N), key=metric)
        ...
        # if active[i]:
                # if rounded_rates[i, 0] >= epsilon[i]:
                #     active[i] = False
                #     continue
                #
                new_rates = deepcopy(rounded_rates[:, t])
                new_rates[i] = increment_in_set(
                    rounded_rates[i, t],allowable_set
                )
                temp_rounded_rates = deepcopy(rounded_rates)
                temp_rounded_rates[:,t] = new_rates
                if (
                    np.sum(new_rates) <= peak_limit
                    and timestep*voltage*sum(temp_rounded_rates[i,:]) <= epsilon[i]
                    and t<d[i]
                    and not rounded_rates[i,t] == max(allowable_set)
                    #and infrastructure_constraints_feasible(new_rates, infrastructure)
                ):
                    rounded_rates[:, t] = new_rates
                else:
                    active[i] = False
